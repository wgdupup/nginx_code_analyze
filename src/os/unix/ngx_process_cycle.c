...
/*子进程循环：
（1）收到特定信号，置标志位ngx_exiting、ngx_terminate、ngx_quit、ngx_reopen，然后调用特定函数
（2）不断调用ngx_process_events_and_timers，该函数会不断尝试获取accept锁，获取成功则接受新连接，以及处理已经建立连接的读写事件
*/
static void ngx_worker_process_cycle(ngx_cycle_t *cycle, void *data)
{
