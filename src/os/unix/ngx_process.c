    ...
    char *name, ngx_int_t respawn)
{
    ...
    if (respawn >= 0) {/*需要重启的进程*/
        s = respawn;

    } else {/*新进程*/
        for (s = 0; s < ngx_last_process; s++) {/*找到一个可放新进程的槽*/
            if (ngx_processes[s].pid == -1) {
                break;
            }
        }

        if (s == NGX_MAX_PROCESSES) {/*超过最大可执行进程数*/
...
static void ngx_process_get_status(void)
{
    ...
    for ( ;; ) {
        ...
        if (pid == -1) {
            ...
            if (err == NGX_EINTR) {//被中断
                continue;
            }

            if (err == NGX_ECHILD && one) {//没有子进程需要被等待
                return;
            }
        ...
        ngx_unlock_mutexes(pid);//释放该子进程持有的所有互斥锁
    }
}
...
void ngx_debug_point(void)
{
    ...
    case NGX_DEBUG_POINTS_STOP:
        raise(SIGSTOP); // 发送SIGSTOP信号暂停进程
        break;

    case NGX_DEBUG_POINTS_ABORT:// 发送SIGABRT信号生成核心转储
        ngx_abort();
    }
}
