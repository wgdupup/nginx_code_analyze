...
typedef struct {
    ngx_uint_t  events;//最大事件数
    ngx_uint_t  aio_requests;//最大AIO并发数
} ngx_epoll_conf_t;
...
events {
    use epoll;
    epoll_events 16384;      # 对应 ngx_epoll_conf_t.events
    aio_requests 4096;       # 对应 ngx_epoll_conf_t.aio_requests
}
*/
...
static ngx_int_t ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    /*如果nevents数量小于epcf->events，那么释放event链表，然后重新分配内存*/
    ...
    ngx_event_actions = ngx_epoll_module_ctx.actions;/*事件处理函数*/
...
static ngx_int_t ngx_epoll_add_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    events = (uint32_t) event;/*初始化事件类型*/

    if (event == NGX_READ_EVENT) {/*读事件*/
        e = c->write;/*获取对立事件*/
        prev = EPOLLOUT;/*记录对立事件类型*/
#if (NGX_READ_EVENT != EPOLLIN|EPOLLRDHUP)
        events = EPOLLIN|EPOLLRDHUP;/*设置为读事件以及对端关闭检测*/
#endif
    ...
    if (e->active) {
        op = EPOLL_CTL_MOD;/*如果对立事件已经注册，那么修改现有事件*/
        events |= prev;/*合并对立事件*/
    } else {
        op = EPOLL_CTL_ADD;/*若对立事件未注册，添加新事件*/
    }
...
/*将事件从epoll中删除，需要检查对立事件，如果对立事件不存在，那么直接删除，否则只是修改*/
static ngx_int_t ngx_epoll_del_event(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags)
{
    ...
    if (e->active) {/*若对立事件仍活跃，修改事件*/
        op = EPOLL_CTL_MOD;
        ee.events = prev | (uint32_t) flags;
        ee.data.ptr = (void *) ((uintptr_t) c | ev->instance);

    } else {/*若对立事件也不活跃，完全删除事件*/
        op = EPOLL_CTL_DEL;
        ee.events = 0;
        ee.data.ptr = NULL;
    }
...
/*将连接c的所有读写事件添加到epoll中，同时设置为et模式*/
static ngx_int_t ngx_epoll_add_connection(ngx_connection_t *c)
{
    struct epoll_event  ee;
    /*存储连接指针和实例标志*/
    ...
    c->read->active = 1;/*标记读事件为活跃状态*/
    c->write->active = 1;/*标记写事件为活跃状态*/
...
static ngx_int_t ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    if (err) {
        if (err == NGX_EINTR) {/*被信号中断*/

            if (ngx_event_timer_alarm) {
                ngx_event_timer_alarm = 0;
                return NGX_OK;/*定时器闹钟触发，直接返回*/
            }
    ...
    for (i = 0; i < events; i++) {
        c = event_list[i].data.ptr;/*取出fd关联的连接*/

        instance = (uintptr_t) c & 1;
        c = (ngx_connection_t *) ((uintptr_t) c & (uintptr_t) ~1);

        rev = c->read;/*获取连接的读事件对象*/
        /*检查连接是否已关闭或状态不一致*/
        ...
        revents = event_list[i].events;/*获取就绪事件类型*/
        ...
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & NGX_POST_EVENTS) {/*延迟处理*/
                queue = rev->accept ? &ngx_posted_accept_events
                                    : &ngx_posted_events;
                /*如果为accept事件，那么添加到ngx_posted_accept_events
                否则添加到ngx_posted_events
                */
                ngx_post_event(rev, queue);

            } else {/*直接处理*/
                rev->handler(rev);/*调用读事件回调函数*/
            }
        }
