...
void ngx_event_accept(ngx_event_t *ev)
{
    ...
    if (!(ngx_event_flags & NGX_USE_KQUEUE_EVENT)) {
        ev->available = ecf->multi_accept;/*设置ev->available接受连接次数*/
    }
    ...
    do {
...
#else
        s = accept(lc->fd, &sa.sockaddr, &socklen);/*获取新连接*/
#endif

        if (s == (ngx_socket_t) -1) {
            err = ngx_socket_errno;

            if (err == NGX_EAGAIN) {/*非阻塞下，表明不是出错，只是还没就绪*/
            ...
            } else if (err == NGX_EMFILE || err == NGX_ENFILE) {/*文件描述符耗尽，严重错误*/
                level = NGX_LOG_CRIT;
            }
            ...
            if (err == NGX_ECONNABORTED) {
                ...
                if (ev->available) {/*还有剩余接受次数，继续尝试接受其他连接*/
                    continue;
                }
            }
            /*文件描述符耗尽*/
...
#endif
        /*ngx_accept_disabled 动态调整 worker 进程接受连接的频率，避免某个进程负载过高
        ...
        *log = ls->log;
        ...
        c->recv = ngx_recv;/*接受函数*/
        c->send = ngx_send;/*发送函数*/
        c->recv_chain = ngx_recv_chain;/*批量接受*/
        c->send_chain = ngx_send_chain;/*批量发送*/
        ...
        rev = c->read;/*连接关联的读事件*/
        wev = c->write;/*连接关联的写事件*/

        wev->ready = 1;/*建立连接时写事件就是就绪*/
...
/*获取accept锁，除了注册读事件失败，否则不会释放accept锁*/
ngx_int_t ngx_trylock_accept_mutex(ngx_cycle_t *cycle)
{
    if (ngx_shmtx_trylock(&ngx_accept_mutex)) {
        ...
        ngx_accept_events = 0;/*置0*/
        ngx_accept_mutex_held = 1;/*设置标志位，表明获取到了锁*/
    ...
    if (ngx_accept_mutex_held) {/*已经获取过锁，那么将其从事件模型中删除*/
