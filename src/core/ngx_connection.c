...
ngx_connection_t * ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    instance = rev->instance;/*用于区分事件是否过期*/

    ngx_memzero(rev, sizeof(ngx_event_t));
    ngx_memzero(wev, sizeof(ngx_event_t));

    rev->instance = !instance;/*事件翻转，避免处理旧事件*/
    wev->instance = !instance;

    rev->index = NGX_INVALID_INDEX;
    wev->index = NGX_INVALID_INDEX;

    rev->data = c;// 读事件关联到当前连接
    wev->data = c;// 写事件关联到当前连接
...
/*回收一些连接，当空闲连接数小于最大连接数的1/16或者存在可复用连接时*/
static void ngx_drain_connections(ngx_cycle_t *cycle)
{
    ngx_uint_t         i, n;
    ngx_queue_t       *q;
    ...
    c = NULL;
    /*回收连接数默认是可复用连接数的1/8，最少回收1个连接，最多回收32个连接*/
    ...
    /*如果回收后无空闲连接，且最后一个连接为可复用，那么强制回收最后一个连接*/
