...
/*
使用old_cycle初始化一个新cycle
（1）申请新的cycle结构体

（2）通过old_cycle进行一些成员变量初始化

（3）统计打开文件数量并进行相应的存储链表初始化

（4）统计共享内存数量并进行相应的存储链表初始化

（5）统计监听对象个数并初始化存储数组

（6）配置文件解析

（7）模块配置结构体拷贝，创建和初始化核心模块的配置文件

（8）打开新文件，并初始化文件描述符，并将该文件描述符设置为cloexec？
...
（13）释放不在使用的打开文件
*/
ngx_cycle_t * ngx_init_cycle(ngx_cycle_t *old_cycle)/*cycle初始化*/
{
    void                *rv, *data;
    char               **senv;
    ngx_uint_t           i, n;
    ...
    file = part->elts;
    /*打开新文件，并初始化文件描述符，并将该文件描述符设置为cloexec*/
    for (i = 0; /* void */ ; i++) {
        ...
        for (n = 0; n < cycle->listening.nelts; n++) {
            ...
            for (i = 0; i < old_cycle->listening.nelts; i++) {
                    ...
                    == NGX_OK)
                {
                    ...
                    nls[n].previous = &ls[i];
                    ls[i].remain = 1;/*如果没有匹配的地址后续就会被释放掉了*/
...
ngx_int_t ngx_create_pidfile(ngx_str_t *name, ngx_log_t *log)
{
    ...
    file.name = *name;
    file.log = log;
    // NGX_FILE_CREATE_OR_OPEN不清空文件， NGX_FILE_TRUNCATE清空并打开文件
...
/*删除主进程pid文件，如果有新的二进制nginx文件，那么删除旧的pid文件，否则删除当前pid文件*/
