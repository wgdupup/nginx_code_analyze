...
int ngx_cdecl 
main(int argc, char *const *argv)/*程序入口*/
{
    ngx_buf_t        *b;
    ngx_log_t        *log;
    ngx_uint_t        i;
...
ngx_pid_t ngx_exec_new_binary(ngx_cycle_t *cycle, char *const *argv)
{
    ...
    ctx.argv = argv;//继承当前进程的参数

    n = 2;
    env = ngx_set_environment(cycle, &n);//获取基础环境变量
    if (env == NULL) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
            ...
            case 's':
                ...
                if (ngx_strcmp(ngx_signal, "stop") == 0//快速停止服务
                    || ngx_strcmp(ngx_signal, "quit") == 0//优雅停止服务
...
static char * ngx_set_user(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    /*
        获取GID
        （1）若配置中只指定了用户名（如 user nobody;），则默认使用该用户的主组。
        （2）若同时指定了组名（如 user nobody nogroup;），则使用指定的组。
    */
...
{
#if (NGX_HAVE_CPU_AFFINITY)//判断是否支持cpu亲和性设置
    ngx_core_conf_t  *ccf = conf;
    ...
    /*自定义绑定核心，比如worker_cpu_affinity 0001 0010; 进程1绑定到核心0，进程2绑定到核心1*/
...
/*设置工作进程数，auto表示根据cpu核心数设置，否则根据用户设置设置*/
