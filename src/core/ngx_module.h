...
/*模块对应的结构体*/
struct ngx_module_s {
    ngx_uint_t            ctx_index;//// 模块上下文索引（用于区分同类模块）
    ...
    void                 *ctx;//模块上下文指针
    ngx_command_t        *commands;//模块支持的配置命令
    ngx_uint_t            type;//模块类型，主要包括core、http、event、mail、stream等

    ngx_int_t           (*init_master)(ngx_log_t *log);//初始化主进程时调用

    ngx_int_t           (*init_module)(ngx_cycle_t *cycle);//初始化模块时调用

    ngx_int_t           (*init_process)(ngx_cycle_t *cycle);//初始化工作进程时调用
    ngx_int_t           (*init_thread)(ngx_cycle_t *cycle);//初始化线程时调用
...
*/
typedef struct {
    ngx_str_t             name;//核心配置名称
    void               *(*create_conf)(ngx_cycle_t *cycle);//创建配置结构体回调函数
    char               *(*init_conf)(ngx_cycle_t *cycle, void *conf);//初始化配置回调函数
} ngx_core_module_t;
